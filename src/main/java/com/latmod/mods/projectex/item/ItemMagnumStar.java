...
public class ItemMagnumStar extends ItemPE implements IItemEmc {
    @SideOnly(Side.CLIENT)
    private IIcon[] icons;

    public ItemMagnumStar() {
        setHasSubtypes(true);
        setMaxDamage(0);
        setMaxStackSize(1);
    ...
    public String getUnlocalizedName(ItemStack stack) {
        ...
        return "item.projectex.magnum_star_" + StarTier.byMeta(meta).getName();
    }

    @Override
    public String getItemStackDisplayName(ItemStack stack) {
        int meta = stack != null ? stack.getItemDamage() : 0;
        return StatCollector.translateToLocal("item.projectex.magnum_star_" + StarTier.byMeta(meta).getName() + ".name");
    }
    ...
    public void registerIcons(IIconRegister register) {
        ...
        for (int i = 0; i < StarTier.VALUES.length; i++) {
            icons[i] = register.registerIcon("projectex:magnum_star_" + StarTier.VALUES[i].getName());
        }
    }
    ...
    public double getMaximumEmc(ItemStack stack) {
        int meta = stack != null ? stack.getItemDamage() : 0;
        return StarTier.byMeta(meta).getMagnumMaxEmc();
    }
}
