...
public class ItemColossalStar extends ItemPE implements IItemEmc {
    @SideOnly(Side.CLIENT)
    private IIcon[] icons;

    public ItemColossalStar() {
        setHasSubtypes(true);
        setMaxDamage(0);
        setMaxStackSize(1);
    ...
    public String getUnlocalizedName(ItemStack stack) {
        ...
        return "item.projectex.colossal_star_" + StarTier.byMeta(meta).getName();
    }

    @Override
    public String getItemStackDisplayName(ItemStack stack) {
        int meta = stack != null ? stack.getItemDamage() : 0;
        return StatCollector.translateToLocal("item.projectex.colossal_star_" + StarTier.byMeta(meta).getName() + ".name");
    }
    ...
    public void registerIcons(IIconRegister register) {
        ...
        for (int i = 0; i < StarTier.VALUES.length; i++) {
            icons[i] = register.registerIcon("projectex:colossal_star_" + StarTier.VALUES[i].getName());
        }
    }
    ...
    public double getMaximumEmc(ItemStack stack) {
        int meta = stack != null ? stack.getItemDamage() : 0;
        return StarTier.byMeta(meta).getColossalMaxEmc();
    }
}
